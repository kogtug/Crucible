...
/**
 * Runs every check against an already-connected harness.
 * A single check throwing is treated as a failure of that check, not a
 * crash of the whole scan - one misbehaving server should never take down
 * the report for the checks that did run cleanly.
 */
export async function runChecks(
  ...
  for (const check of checks) {
    try {
      results.push(await check.run(harness));
    } catch (err) {
      results.push({
        id: check.id,
        title: check.title,
