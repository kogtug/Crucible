...
export function serverAdvertisesTasks(discoverResult: Record<string, unknown>): boolean {
  const extensions = (discoverResult.capabilities as { extensions?: Record<string, unknown> } | undefined)?.extensions;
...
export function tasksNotAdvertisedResult(check: { id: string; title: string; specRef?: string }): CheckResult {
  return {
    id: check.id,
    title: check.title,
    status: "warn",
